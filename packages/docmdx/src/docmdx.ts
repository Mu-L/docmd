...
function resolveDocmdBin(): string | null {
  ...
  try {
    ...
    const meta = JSON.parse(readFileSync(corePkgPath, 'utf-8')) as { bin?: Record<string, string> };
  ...
  try {
    ...
    if (existsSync(monoCoreBin)) return monoCoreBin;
